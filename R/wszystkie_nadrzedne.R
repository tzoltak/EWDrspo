...
#' temat placówki. Kolejne elementy listy zawierają dane dotyczące zespołów
...
wszystkie_nadrzedne <- function(placowka) {
  ...
  rspoNadrzedneVec =
    sapply(ret, function(x) {return(x[colnames(x) == "Numer RSPO"])})[-1]
  ...
  while (ind <= length(rspoNadrzedneVec)) {
    ...
    rspoNadrzedneVec = c(rspoNadrzedneVec,
                         sapply(daneNad, function(x) {return(x[colnames(x) == "Numer RSPO"])}))
    ind = ind + 1
  }
  return(ret)
}
