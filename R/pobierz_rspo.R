...
#' }
#' @param rownolegle wartość logiczna - czy dane z poszczególnych województw
#' mają być zbierane równolegle (z użyciem pakietu \code{snow})?
#' @return data frame z bazą RSPO
#' @export
#' @importFrom snow makeCluster parLapply stopCluster
pobierz_rspo = function(rownolegle=TRUE) {
  ...
  if (rownolegle) {
    ...
    if (!rownolegle) {
      warning(strwrap("Pakiet 'snow' jest niedostępny. Dane dotyczące poszczególnych województw będą pobierane sekwencyjnie, a nie równolegle.", prefix = " "),
              immediate. = TRUE)
    }
  }
  # odpalamy ściąganie
